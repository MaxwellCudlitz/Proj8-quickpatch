...
def index():
    ...
    if 'begin_date' not in flask.session or 'selected_calendars' not in flask.session:
        init_session_values()
...
def init_session_values():
    ...
    flask.session['selected_calendars'] = []
...
def get_selected_cals():
    """
    returns a JSON object containing the session selected
    calendars.
    """
    app.logger.debug("retrieving session calendars")
    if('selected_calendars' in flask.session):
        ret = flask.session['selected_calendars']
    else:
        ret = []
    return flask.jsonify({'cals' : ret})


@app.route('/api/session/_togglecal', methods=['POST'])
def api_sess_toggleCal():
    """
    Toggles the selected calendar's state in the session
    """

    calid = request.get_json()['id']    
    app.logger.debug("Toggling calendar {}".format(calid))

    if('selected_calendars' not in flask.session):
        flask.session['selected_calendars'] = []
